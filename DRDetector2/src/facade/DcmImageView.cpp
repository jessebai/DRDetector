...
QVector<double> DcmImageView::countDw(double x1, double x2, double y1, double y2){
    ...
    double lowPixelsRangePercentage = 0.2;//极小值判定范围0.1*len/13
    double maxDifferencePercentage = 0.08;//极小值与范围内最大值的差值，与整个曲线最小最大值的差值，的比例
    double neighbourLowPixelsRangePercentage = 0.5;//判断两个极小值是否属于双线，小于该值是
...
//    qDebug() << "bottomkLimit: " << bottomkLimit;
    ...
    //寻找局部极小值，要求：1.该点在局部范围内极小。2.该点与局部最大值相差较大。3.与上一个极小值距离在范围内
    ...
    for(int i = 0; i < lowPixels.size()-1; i++){
        if(lowPixelsIndex[i+1] - lowPixelsIndex[i] < neighbourLowPixelsRange){
            ...
            //如果不是双线的极小值，需要判断是不是出了异常点还是接下来都没有双线的极小值
            //寻找该点（1.一定范围内2.值接近）的极小值（3.小范围内极小4.所有可能点里最小）
    ...
    //测试发现基本双线可以找齐，单线不一定能找齐，所以单线坐标需要推理一下
    ...
    //根据上述结果计算dip = (A+B-2C)/(A+B)
    //同时给出各个dip文字的中心位置，双线位于极大值处，单线位于极小值处，没有线就算一个位置
    QVector<double> dip;
